...
public class OtherAlgorithmsTest {
    ...
    //Алгоритм работает примерно для чисел меньше <25, так как
    @Test
    @DisplayName("Проверка на простоту числа через теорему ферма")
    void checkPrimeNumberTest() {
        Assertions.assertTrue(Algorithms.checkPrimeNumber(11));
        Assertions.assertFalse(Algorithms.checkPrimeNumber(22));
        Assertions.assertTrue(Algorithms.checkPrimeNumber(13));
        Assertions.assertTrue(Algorithms.checkPrimeNumber(41));
    }
}
